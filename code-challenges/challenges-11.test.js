...
const toLastNames = people => {
  ...
  let spaceNoted = ' ';
  people.map(person => {
    nameArray.push(person.firstName + spaceNoted + person.lastName);
  })
  return nameArray;
};
...
const validatePin = (pin) => {
  // Solution code here...
  const regExpress = /\b\d{4}\b/;
  if (regExpress.test(pin)) {
    return true;
  } else {
    return false;
  }
...
const validateEmail = (email) => {
  // Solution code here...
  let regExpress = /^\w+(|.)\w+@\w+.(com|net|org)$/;
  if(regExpress.test(email)) {
   return true;
  } else {
    return false;
  }
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  const phoneExp = /\(?\d{3}\)?[\s-]?\d{3}[\s-]?\d{4}$/;
  const validCheck = phoneExp.test(phoneNumber); 
};
