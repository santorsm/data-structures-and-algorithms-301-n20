...
const templatingWithMustache = () => {
  ...
  characters.forEach(character => {
    let renderStache = Mustache.render($template, character);
    result.push(renderStache);
  })
  return result;
};
...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  for (let parent in arr){
    if (arr[parent].name === character){
      return Object.values(arr[parent].children).length > 0;
    }
  };
};
...
const hasChildrenEntries = (arr, character) => {
  // Solution code here...
  for (let dataParent in arr){
    if (arr[dataParent].name === character){
      return Object.entries(arr[dataParent].children).length > 0;
    }
  };
};
