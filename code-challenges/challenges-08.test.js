...
function sayHello(request, response){
  // Solution code here...
  response.status(404).send('Hello from the back-end');
}
...
const oddValues = (arr) => {
  // Solution code here...
  let oddIntegers = arr.filter((integerValue) => integerValue % 2 !== 0);
  return oddIntegers;
};
...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  let wordsWithVowels = arr.filter((vowels) => /[aeiou]/.test(vowels));
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  let onlyinSecond = arr.filter((i) => !forbiddenValues.includes(i));
  return onlyinSecond;
};
