...
const createServer = () => {
  ...
  function errorHandler(request, response){
    response.status(404).send('Sorry, Something is rotten in the state of Denmark');
  }

  function aboutmeHandler (request, response){
    response.status(200).send('I\'m a former pilot who does\'nt know how to use response.send');
  }

  function foodHandler (request, response){
    response.status(200).send('broccoli rabe, pasta y fagoli, calamari, manicotti');
  }
